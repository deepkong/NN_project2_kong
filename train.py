...
from model.music_transformer import MusicTransformer, MusicDiscriminator, MusicPatchDiscriminator
...
CSV_HEADER = ["Epoch", "Learn rate", "Avg Eval loss", "Eval accuracy"]
...
def main():
    ...
    train_dataset, val_dataset = create_epiano_datasets(args.input_dir, args.max_sequence)
    ...
    # added
    # model_D = MusicDiscriminator(n_layers=1, num_heads=2).to(get_device())
    model_D = MusicPatchDiscriminator(n_layers=1, num_heads=2).to(get_device())
    ...
    # unc = UncertaintyLoss(2)
    # bce = BCELoss()

    ##### Optimizer #####
    opt = Adam(model.parameters(), lr=lr, betas=(ADAM_BETA_1, ADAM_BETA_2), eps=ADAM_EPSILON)
    opt_D = Adam(model_D.parameters(), lr=lr, betas=(ADAM_BETA_1, ADAM_BETA_2), eps=ADAM_EPSILON)

    # unc.cuda()

    if(args.lr is None):
        lr_scheduler = LambdaLR(opt, lr_stepper.step)
        lr_scheduler_D = LambdaLR(opt_D, lr_stepper.step)
    else:
        lr_scheduler = None
        lr_scheduler_D = None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if(not args.no_tensorboard):
            ...
            tensorboard_summary.add_scalar("Avg_CE_loss/val", eval_loss, global_step=epoch+1)
            # tensorboard_summary.add_scalar("Accuracy/train", train_acc, global_step=epoch+1)
            tensorboard_summary.add_scalar("Accuracy/val", eval_acc, global_step=epoch+1)
        ...
        with open(results_file, "a", newline="") as o_stream:
            writer = csv.writer(o_stream)
            writer.writerow([epoch+1, lr, eval_loss, eval_acc])
